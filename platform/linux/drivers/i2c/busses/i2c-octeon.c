...
static int octeon_i2c_wait(struct octeon_i2c *i2c)
{
	...
	if (result <= 0 && octeon_i2c_test_iflg(i2c)) {        
        /*Modify this print as debug level. jqiu 2012-03-26. Though this cpu support irq, but it seems when CPU is 
          busy, sometime it will lose the irq, so maybe this is cpu limitiation or octeon SDK limitiation, not print 
          error. And this action won't affect I2C function.*/
		dev_dbg(i2c->dev, "broken irq connection detected, switching to polling mode.\n");
        /*Jqiu 2012-03-27. CN5010 support irq, so here cfg always broken_irq_mode 0.*/
		i2c->broken_irq_mode = 0;
		return 0;
	}
			   ...
			   int num)
{
    ...
    /*Merge from cavium SDK 2.3.0, jqiu 2012-03-26*/
	if (num == 1) {
