...
struct cpm_i2c {
	...
	int irq;
    spinlock_t flags; //add by jcao for i2c transfer timeout 2011.02.23
	int cp_command;
	int freq;
	...
	while (tptr < num) {
		...
		if (pmsg->flags & I2C_M_RD)
			ret = wait_event_timeout(cpm->i2c_wait,
				...
				12 * HZ);/*modified by jcao 2011.2.12 for i2c transfer time out*/
		else
			ret = wait_event_timeout(cpm->i2c_wait,
		...
		if (ret == 0) {
            ...
            if (timeout) 
            {
                cpm2_immr->im_ioport.iop_ppard &= (~0x20000); 
                udelay(10);
                cpm2_immr->im_ioport.iop_pdird |= 0x20000;
                udelay(10);
                for (i = 0;i < 12;i++ )
                {
                	cpm2_immr->im_ioport.iop_pdatd &= (~0x20000);
                	udelay(10);
                	cpm2_immr->im_ioport.iop_pdatd |= 0x20000;
                	udelay(10);
                 }
                cpm2_immr->im_ioport.iop_pdird &= (~0x20000);
                udelay(10);
                cpm2_immr->im_ioport.iop_ppard |= 0x20000;udelay(10);
...
static int __devinit cpm_i2c_setup(struct cpm_i2c *cpm)
{
	...
	if (data && len == 4)
		...
		cpm->freq = 30000; /*modified by jcao 2011.2.25 for i2c transfer time out */ 
	 ...
	 */
	 /*we modify i2c brg clk, and now use 50kHz i2c clock*/
	brg = get_brgfreq() / (32 * 2 * cpm->freq) - 3;
    brg = 0x1c;/*modified by jcao 2011.03.09 for i2c transfer time out */ 
	out_8(&cpm->i2c_reg->i2brg, brg);

    out_8(&cpm->i2c_reg->i2mod, 0x04);/*modified by jcao 2011.03.09 for i2c transfer time out */ 
			 ...
			 const struct of_device_id *match)
{
    ...
    spin_lock_init(&cpm->flags);//add by jcao for i2c transfer timeout 2011.02.23
