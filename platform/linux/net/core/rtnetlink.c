...
{
#if 0 /* m by zhuj for vrf, let all netlink message send to init net vrf */
	//struct sock *rtnl = net->rtnl;
#else
    struct sock *rtnl = init_net.rtnl;
#endif
	int err = 0;
...
#if 1     /* zhuj add at 2010.11.16 for vrf */
	if (tb[IFLA_NET_NS]) {
		struct net *net;
		...
		err = ctc_dev_change_net_namespace(dev, net, ifname);
		put_net(net);
		if (err)
			goto errout;
		modified = 1;
