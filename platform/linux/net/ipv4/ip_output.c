...
int (*mpls_dcn_proc) (struct sk_buff *, __be32 daddr, int forward) = NULL;
void (*mpls_dcn_route) (__be32 daddr, struct rtable** rp) = NULL;
...
#endif
/*add by chenyuqiang for protocol pkt tunneled for bug 14302, 2010-03-09*/
	if (skb->enable_mpls_fwd == 1)
	  {
           skb->enable_mpls_fwd = 2; 
...
/*changed by chenyuqiang for frr support ldp packet by tunnel, 2011-3-9*/
  iph = ip_hdr(skb);

  if(skb->enable_mpls_fwd == 0 && mpls_output && iph )
  {
    if(iph->protocol == (IPPROTO_UDP)) 
    {
      uh =(void *)((unsigned char*)iph + iph->ihl * 4 );
      if(uh && (uh->dest == ntohs(646)))
      {
    ...
    {
      th = (void *)((unsigned char*)iph + iph->ihl * 4 );
      if(th && (th->dest == ntohs(646) || th->source == ntohs(646)))
      {
        skb->enable_mpls_fwd = 1;
      }
    }
  ...
  if(skb->enable_mpls_fwd == 1 && mpls_output && iph )
  {
    if(iph->protocol == (IPPROTO_UDP)) 
    {
...
/*
    if(skb->enable_mpls_fwd == 1)
	  ...
	  */
/*end of changed by chenyuqiang*/

    if (mpls_dcn_route)
    {   
        mpls_dcn_route(iph->daddr, &rth);
        if (rth)
        {
            skb->enable_mpls_fwd = 1;
        }
...
int l2_send_frame (struct sk_buff *sk)
{
  ...
  {
    retval = dst->neighbour->output(sk);
  }
  else 
  {
    kfree_skb(sk);
...
int mpls_ip_output (struct sk_buff *skb)
{
  ...
  if (1 == ret )
  {
    ...
    if(dst->dev)
    {
        /* added to avoid interface shutdown but still forwarding packet by chengw 2011-4-6 */
        if((dst->dev->reg_state != NETREG_REGISTERED) || !(dst->dev->flags & IFF_UP) )
        {
            kfree_skb (skb);
        }
        else
...
/* modified by yaom for mpls dcn, 2011-10-20 */
void mpls_register_dcn_proc_hdlr (int (*fn) (struct sk_buff *skb, __be32 daddr, int forward))
{
  if (fn)
    mpls_dcn_proc = fn;
}

void mpls_unregister_dcn_proc_hdlr (void)
{
  mpls_dcn_proc = NULL;
}

void mpls_register_dcn_route_hdlr (void (*fn) (__be32 daddr, struct rtable** rp))
{
  if (fn)
    mpls_dcn_route = fn;
}

void mpls_unregister_dcn_route_hdlr (void)
{
  mpls_dcn_route = NULL;
}
...
/* modified by yaom for mpls dcn, 2011-10-20 */
EXPORT_SYMBOL(mpls_register_dcn_proc_hdlr);
EXPORT_SYMBOL(mpls_unregister_dcn_proc_hdlr);
EXPORT_SYMBOL(mpls_register_dcn_route_hdlr);
EXPORT_SYMBOL(mpls_unregister_dcn_route_hdlr);
