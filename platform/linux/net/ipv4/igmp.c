...
/*
 *	get multicast groups socket joined, added by tanker
 */
int ip_sock_mc_get(struct sock *sk, char __user *optval, int num)
{
    ...
    struct ip_mc_list *ifmca;
    struct net *net = sock_net(sk);
    
    copy_from_user(&ifindex, optval, sizeof(int));
    addr = (struct in_addr*)(optval + sizeof(int));
    ...
    if(idev)
    {
        read_lock_bh(&idev->mc_list_lock);
    	for(ifmca = idev->mc_list; ifmca != NULL; ifmca = ifmca->next)
        {
            if(count < num)
                copy_to_user(addr + count, &ifmca->multiaddr, sizeof(struct in_addr));
            
			count++;
    	}
        read_unlock_bh(&idev->mc_list_lock);
        in_dev_put(idev);
    }
    dev_put(dev);

	return count * sizeof(struct in_addr) + sizeof(int);
}
