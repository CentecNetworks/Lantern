...
static struct ipv4_devconf ipv4_devconf_dflt = {
	.data = {
		...
		[NET_IPV4_CONF_ICMP_UNREACH_ENABLE - 1] = 1,
	},
};
...
static struct in_ifaddr *rtm_to_ifaddr(struct net *net, struct nlmsghdr *nlh)
{
	...
	err = -ENOBUFS;
#if 0  /* m by zhuj for vrf at 2010.11.17 */
	if (in_dev == NULL)
		goto errout;
#else
	if (in_dev == NULL)
			 ...
			 void *ptr)
{
	...
	case NETDEV_UP:
		if (!inetdev_valid_mtu(dev->mtu))
			break;
		if ((dev->flags & IFF_LOOPBACK) && (!strcmp(dev->name, "lo"))) {
			struct in_ifaddr *ifa;
			if ((ifa = inet_alloc_ifa()) != NULL) {
				ifa->ifa_local =
...
#if 0   /*Modified by xgu for bug 15206, 2011-7-18*/     
        else if((dev->flags & IFF_LOOPBACK) && (dev->nd_net->id != 0)){
            dev_put(dev);
        }
#endif        
		ip_mc_up(in_dev);
		break; /*m by zhuj at 2010-11-23, when net device up, doesn't send arp request*/	
		/* fall through */
...
static struct devinet_sysctl_table {
	...
	.devinet_vars = {
		...
		DEVINET_SYSCTL_RW_ENTRY(ICMP_UNREACH_ENABLE, "icmp_unreach_enable"),
