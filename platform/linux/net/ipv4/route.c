...
extern void (*mpls_dcn_route) (__be32 daddr, struct rtable** rp);
...
#if 0 /*deleted by dingyi for bug12020, 2010-05-13*/
			   ...
			   struct rtable **result)
{
    ...
    /*for bug 7012*/
    if ((in_dev != out_dev) && !IN_DEV_FORWARD(out_dev)) {
        err = -EHOSTUNREACH;
        goto cleanup;
    }
	...
	if (out_dev == in_dev && err && !(flags & (RTCF_NAT | RTCF_MASQ)) &&
        (/*IN_DEV_SHARED_MEDIA(out_dev) ||*/ /*qmiao for bug 2522*/
...
static int __mkroute_output(struct rtable **result,
			    ...
			    unsigned flags,
			    int dcn_flag)/* modified by yaom for mpls dcn, 2011-10-20 */
{
	struct rtable *rth;
	struct in_device *in_dev;
    ...
    {
        rth->fl.oif = fl->oif;
        rth->rt_iif = fl->oif ? : dev_out->ifindex;
    }
    else
    {
...
{
	struct rtable *rth = NULL;
	int err = __mkroute_output(&rth, res, fl, oldflp, dev_out, flags, dcn_flag);
	unsigned hash;
	if (err == 0) {
                ...
                struct flowi *oldflp)
{
    ...
    if (mpls_dcn_route)
    {   
        mpls_dcn_route(oldflp->fl4_dst, &rth_ftn);
        if (rth_ftn)
        {
            /* match a ftn route */
            dcn_flag = 1;

            /* if there is no source address, should assign management address to source */
            if (!oldflp->fl4_src)
            {
                /* if management adress is not configured, should return unreach */
                if (!rth_ftn->rt_src)
                {
                    err = -ENETUNREACH;
                    goto out;
                }
                oldflp->fl4_src = rth_ftn->rt_src;
                fl.fl4_src = rth_ftn->rt_src;
            }

            /* get ftn route out dev*/
            dev_out = rth_ftn->u.dst.dev;

            /* update out interface, will not change oif of oldflp coz that's also used for hash lookup */
            fl.oif = dev_out->ifindex;
            dev_hold(dev_out);

            /* assign route type for ftn route --> unicast*/
            res.type = RTN_UNICAST;

            /* directly go to make route, avoid fib looking up */
            goto make_route;
        }
    }
...
make_route:
    err = ip_mkroute_output(rp, &res, &fl, oldflp, dev_out, flags, dcn_flag);
...
int __ip_route_output_key(struct net *net, struct rtable **rp,
			  struct flowi *flp)/* modified by yaom for mpls dcn, 2011-10-20 */
{
	unsigned hash;
	struct rtable *rth;
