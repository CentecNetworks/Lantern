...
/* merged by zhuj at 2010.10.12 */    
...
#else
	rd_len = min_t(unsigned int,
		     IPV6_MIN_MTU-sizeof(struct ipv6hdr)-len, (skb->len + 8 + 7) & (~0x7));
#endif
	rd_len &= ~0x7;
	len += rd_len;
	...
	*(opt++) = (rd_len >> 3);
    ...
    /* set the byte filled been 0, merged by zhuj at 2010.10.12  */
    if(rd_len - 8 > skb->len)
        memset(opt + skb->len, 0, rd_len - 8 - skb->len);
...
int ndisc_rcv(struct sk_buff *skb)
{
    ...
    /* merged by zhuj at 2010.10.12 
     * <peth_priv.h>
     * #define PETH_IFT_XXX 0x5AXX
     *
     * NDP messages incoming from PETH_IFT_XXX interfaces will be processed
     * by NSM/NDP module which receives NDP messages through IP raw socket.
     */
    if ((dev->priv_flags & 0xFF00) == 0x5A00)
        return 0;
