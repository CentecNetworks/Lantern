 ...
 */
int ipv6_sock_mc_get(struct sock *sk, char __user *optval, int num)
{
    ...
    addr = (struct in6_addr*)(optval + sizeof(int));
    ...
    if(idev)
    {
    	...
    	for(ifmca = idev->mc_list; ifmca; ifmca = ifmca->next)
        {
            if(count < num)
                copy_to_user(addr + count, &ifmca->mca_addr, sizeof(struct in6_addr));
	...
	return count * sizeof(struct in6_addr) + sizeof(int);
}
