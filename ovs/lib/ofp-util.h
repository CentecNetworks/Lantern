...
#define NXAST_ACTION(ENUM, STRUCT, EXTENSIBLE, NAME)    \
    ...
    struct STRUCT *ofputil_put_##ENUM(struct ofpbuf *);
#ifdef _OFP_CENTEC_
#define OFPTFPT13_PROPERTY(ENUM, STRUCT)              \
    void ofputil_init_##ENUM(struct STRUCT *);          \
    struct STRUCT *ofputil_put_##ENUM(struct ofpbuf *);
#endif
#include "ofp-util.def"
...
#ifdef _OFP_CENTEC_
#define OFPPC_PORT_CONFIG_CHECK(config, mask, flag)        \
    all_check_bit = flag;        \
    if ((config & (~all_check_bit)) || (mask & (~all_check_bit)))       \
...
struct ofputil_table_feature_prop {
    ...
    uint8_t *elements;       /* Series of database. */
    size_t elements_len;    /* Length of elements, in bytes. */
};
...
#define OFUTIL_MAX_QUEUE_VALUE   8

struct ofputil_queue_config_request {
    uint16_t port;
...
struct ofputil_queue_get_config_reply {
    ovs_be32 port;
    uint8_t pad[4];
...
struct ofputil_queue_config_s{
    uint32_t port_id;
    uint16_t min_rate[OFUTIL_MAX_QUEUE_VALUE];
    uint16_t max_rate[OFUTIL_MAX_QUEUE_VALUE];
};
