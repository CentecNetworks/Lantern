...
#ifdef _OFP_CENTEC_
static void
parse_vlans(struct ofpbuf *b, struct flow *flow)
{
    int vlan_tag_count = 0;
    struct qtag_prefix {
    ...
    while ((ethtype == htons(ETH_TYPE_VLAN) ||
            ...
            ethtype == htons(ETH_TYPE_VLAN_8021Q_9100) ||
            ethtype == htons(ETH_TYPE_VLAN_8021Q_9200)) &&
        ...
        } else if (vlan_tag_count >=1 && vlan_tag_count < MAX_VLAN_TAGS) {
            flow->vlan_qinq_tci[vlan_tag_count - 1] = qp->tci | htons(VLAN_CFI);
            flow->vlan_qinq_tpid[vlan_tag_count - 1] = qp->eth_type;
        }
        ethtype = *((ovs_be16 *) b->data);
        vlan_tag_count++;
    }
    flow->org_vlan_tag_count = vlan_tag_count;
}
#else
static void
             ...
             struct flow *flow)
{
    ...
    if (eth->eth_type == htons(ETH_TYPE_VLAN) ||
            ...
            eth->eth_type == htons(ETH_TYPE_VLAN_8021Q_9100) ||
            eth->eth_type == htons(ETH_TYPE_VLAN_8021Q_9200)) {
       parse_vlans(&b, flow);
    }
#endif
