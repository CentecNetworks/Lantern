...
ovsrec_bridge_set_protocols(const struct ovsrec_bridge *row, char **protocols, size_t n_protocols)
{
    ...
    ovs_assert(inited);
    datum.n = n_protocols;
    datum.keys = xmalloc(n_protocols * sizeof *datum.keys);
    datum.values = NULL;
...
ovsrec_interface_set_ofport_request(const struct ovsrec_interface *row, const int64_t *ofport_request, size_t n_ofport_request)
{
    ...
    ovs_assert(inited);
    datum.n = n_ofport_request;
    datum.keys = xmalloc(n_ofport_request * sizeof *datum.keys);
    datum.values = NULL;
    for (i = 0; i < n_ofport_request; i++) {
        datum.keys[i].integer = ofport_request[i];
    }
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_INTEGER, OVSDB_TYPE_VOID);
    ovsdb_idl_txn_write(&row->header_, &ovsrec_interface_columns[OVSREC_INTERFACE_COL_OFPORT_REQUEST], &datum);
}
