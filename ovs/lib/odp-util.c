...
#ifdef _OFP_CENTEC_
    case OVS_ACTION_ATTR_GROUP: return sizeof(uint32_t);
#endif
...
format_odp_action(struct ds *ds, const struct nlattr *a)
{
    ...
    case OVS_ACTION_ATTR_GROUP:
        ds_put_format(ds, "group=0x%"PRIx32, nl_attr_get_u32(a));
        break;
#endif
    case OVS_ACTION_ATTR_UNSPEC:
    case __OVS_ACTION_ATTR_MAX:
                     ...
                     struct flow *flow)
{
    ...
    if (flow->dl_type == htons(ETH_TYPE_VLAN)
            ...
            || flow->dl_type == htons(ETH_TYPE_VLAN_8021Q_9100)
            || flow->dl_type == htons(ETH_TYPE_VLAN_8021Q_9200)) {
        return parse_8021q_onward(attrs, present_attrs, out_of_range_attr,
                                  expected_attrs, flow, key, key_len);
    }
#endif
...
#ifdef _OFP_CENTEC_
static void
commit_vlan_actions(const struct flow *flow, struct flow *base,
                   struct ofpbuf *odp_actions, int vlan_tag_index)
{
    if (flow->vlan_qinq_tci[vlan_tag_index] & htons(VLAN_CFI)) {
        struct ovs_action_push_vlan vlan;
        /* For actions push_vlan:0x8100/0x88a8/0x9100/0x9200, followed by strip_vlan
         * should be a no-op.
         * For actions strip_vlan, followed by push_vlan:0x8100/0x88a8/0x9100/0x9200
         * new vlan header is pushed.
         */
        if (flow->vlan_qinq_tpid[vlan_tag_index] == htons(ETH_TYPE_VLAN) ||
                flow->vlan_qinq_tpid[vlan_tag_index] == htons(ETH_TYPE_VLAN_8021AD) ||
                flow->vlan_qinq_tpid[vlan_tag_index] == htons(ETH_TYPE_VLAN_8021Q_9100) ||
                flow->vlan_qinq_tpid[vlan_tag_index] == htons(ETH_TYPE_VLAN_8021Q_9200)) {
            vlan.vlan_tpid = flow->vlan_qinq_tpid[vlan_tag_index];
        } else {
            vlan.vlan_tpid = htons(ETH_TYPE_VLAN);
        }
        vlan.vlan_tci = flow->vlan_qinq_tci[vlan_tag_index];
        nl_msg_put_unspec(odp_actions, OVS_ACTION_ATTR_PUSH_VLAN,
                          &vlan, sizeof vlan);
    }
    base->vlan_tpid = flow->vlan_tpid;
    base->vlan_qinq_tci[vlan_tag_index] = flow->vlan_qinq_tci[vlan_tag_index];
}
#endif
                   ...
                   struct ofpbuf *odp_actions)
{
    ...
    if (flow->vlan_tci & htons(VLAN_CFI)) {
        ...
        vlan.vlan_tpid = htons(ETH_TYPE_VLAN);
#else
        if (flow->vlan_tpid == htons(ETH_TYPE_VLAN) ||
                flow->vlan_tpid == htons(ETH_TYPE_VLAN_8021AD) ||
                flow->vlan_tpid == htons(ETH_TYPE_VLAN_8021Q_9100) ||
                flow->vlan_tpid == htons(ETH_TYPE_VLAN_8021Q_9200)) {
...
commit_set_port_action(const struct flow *flow, struct flow *base,
                       struct ofpbuf *odp_actions)
{
    /* matching packets' tp_src and tp_dst are not necessary to be non-zero values here */
#ifndef _OFP_CENTEC_
...
{
    int vlan_tag_index;
    int org_vlan_tag_count = 0;

    /* pop all vlans in original packets */
    for (org_vlan_tag_count = 0; org_vlan_tag_count < flow->org_vlan_tag_count; org_vlan_tag_count++) {
        nl_msg_put_flag(odp_actions, OVS_ACTION_ATTR_POP_VLAN);
    }

    for (vlan_tag_index = MAX_VLAN_TAGS - 2; vlan_tag_index >= 0; vlan_tag_index--) {
        commit_vlan_actions(flow, base, odp_actions, vlan_tag_index);
    }
}
#endif
