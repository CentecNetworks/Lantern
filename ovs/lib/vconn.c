...
#ifdef _OFP_CENTEC_
void 
vconn_send_error(struct vconn *vconn, enum ofperr error, char *s);
void
vconn_send_error(struct vconn *vconn, enum ofperr error, char *s)
{
    struct ofpbuf *b;
    ...
    b = ofperr_encode_hello(error, vconn->version, s);
    retval = do_send(vconn, b);
    if (retval) {
        ofpbuf_delete(b);
    }
    ...
    if (!retval && !vconn->recv_any_version) {
        ...
        if (oh->version != vconn->version) {
            ...
            if (ofptype_decode(&type, msg->data)
                ...
                {
                    char s[128];
                    snprintf(s, sizeof s, "Received OpenFlow version 0x%02x != expected 0x%02x",
                            oh->version, vconn->version);
                    vconn_send_error(vconn, OFPERR_OFPBRC_BAD_VERSION, s);
                }
#endif
                ofpbuf_delete(msg);
                retval = EPROTO;
