...
parse_options(int argc, char *argv[])
{
    ...
    vlog_set_verbosity("ANY:syslog:off");
#endif
...
usage(void)
{
    printf("%s: OpenFlow switch management utility\n"
...
#ifdef _OFP_CENTEC_
           "  dump-queue-stats SWITCH [PORT [QUEUE]]  dump queue stats\n"
#else
...
#ifdef _OFP_CENTEC_
           "  dump-table-fea SWITCH       print table features\n"
           ...
           "  dump-queue-config SWITCH [PORT]  dump queue config\n"
#endif
...
#else
    open_vconn(vconn_name, &vconn);
    request = ofpraw_alloc(OFPRAW_OFPST_PORT_DESC_REQUEST, vconn_get_version(vconn), 0);
                       ...
                       struct ofputil_phy_port *pp)
{
    ...
    /* force to fetch port by stats rather than by features,
     * since fetch port list from OFPT_FEATURES_REQUEST message is deprecated in Openflow1.3  */
#ifdef _OFP_CENTEC_
    trunc = true;
#endif
    if (trunc) {
