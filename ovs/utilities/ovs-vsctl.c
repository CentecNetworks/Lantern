...
#ifndef _OFP_CENTEC_
    /* don't print this log message, this log message is out of control by parse_options */
    VLOG(might_write_to_db(argv) ? VLL_INFO : VLL_DBG, "Called as %s", args);
#else
    /* dummy codes, avoid compilation warning: unused function */
    might_write_to_db(argv);
#endif
...
Database commands:\n\
  ...
  set-queue-config PORT QUEUE CIR PIR set the config for queue(percent)\n\
...
#ifdef _OFP_UML_
#define DEFAULT_NETDEV_CPU_PORT "eth104"
#else
#define DEFAULT_NETDEV_CPU_PORT "eth1"
#endif

static bool
is_valid_eth_name(const char * eth_name)
{
    uint32_t slot = 0xffffffff;
    uint32_t port = 0xffffffff;
    int32_t  ret;
    int32_t  i;
    int32_t non_digital_count = 0;

    /* eth-x-x, eth-x-xx*/
    if (strlen(eth_name) != 7 && strlen(eth_name) != 8) {
        return false;
    }

    if (eth_name[0] != 'e' || eth_name[1] != 't'  || eth_name[2] != 'h' || eth_name[3] != '-' || eth_name[5] != '-') {
        return false;
    }

    for (i=0; i<strlen(eth_name); i++) {
        if (0 == isdigit(eth_name[i])) { /* if it is not a digital number */
            non_digital_count++;
        }
    }
    ...
    ret = sscanf(eth_name, "eth-%1u-%2u", &slot, &port);
...
static bool
is_valid_gre_name(const char * netdev_name)
{
    int i = 0;
    uint32_t gre_port = OFP_INVALID_GPORT;
    ...
    /* greX ~ greXXX */
    if (strlen(netdev_name) < 4 || strlen(netdev_name) > 6) {
        return false;
    }

    if (strncmp(netdev_name, "gre", 3) != 0) {
        return false;
    }
    ...
    for (i = 3; i < strlen(netdev_name); i++) {
        if (!isdigit(*(netdev_name+i))) {
            return false;
        }
    }
    if (sscanf(netdev_name, "gre%3u", &gre_port) != 1) {
        return false;
    }
...
static bool
is_valid_ipv4_address(const char * ipv4_address)
{
    int i = 0;
    int dot_count = 0;
    ...
    if (strspn(ipv4_address, "0123456789.") != strlen(ipv4_address))
    {
        return false;
    }

    if (strlen(ipv4_address) < 7 || strlen(ipv4_address) > 15) {
        return false;
    }

    for (i = 0; i < strlen(ipv4_address); i++) {
        if ('.' == ipv4_address[i]) {
            dot_count++;
        }
    }
...
/**
 * note:
 * command shown below will be divided to sub commands: 'add-port' and 'set'.
 * the count of arguments of sub commands will be subtracted by 3, and thus the argc of 'add-port' is 0.
 * to find out the arguments of the other sub command 'set', we need to parse the whole argument of settings[] as workaround.
 * eg. ovs-vsctl add-port br0 gre1 -- set interface gre1 type=gre options:remote_ip=0.0.0.1 options:local_ip=1.0.0.1 options:bind_port=eth-0-1 options:nexthop_mac=0.0.1
 **/
static bool
check_gre_iface_settings(char *settings[], int n_settings)
{
#define CMD_DASH        "--"
#define CMD_SET         "set"
#define CMD_INTERFACE   "interface"
#define CMD_NETDEV_TYPE "type="
#define CMD_REMOTE_IP   "options:remote_ip="
#define CMD_LOCAL_IP    "options:local_ip="
#define CMD_BIND_PORT   "options:bind_port="
#define CMD_NEXTHOP_MAC "options:nexthop_mac="
#define CMD_VLAN_ID     "options:vlan_id="
    ...
    size_t i = 0;
    enum cmd_parse_state state = CMD_STATE_READY;
    char *iface_name = NULL;
    char *netdev_type = NULL;
    char *remote_ip = NULL;
    char *local_ip = NULL;
    char *bind_port = NULL;
    char *nexthop_mac = NULL;
    char *vlan_id_str = NULL;
    int vlan_id = OFP_DEFAULT_VLAN_ID;
    int netdev_type_len = strlen(CMD_NETDEV_TYPE);
    int remote_ip_len = strlen(CMD_REMOTE_IP);
    int local_ip_len = strlen(CMD_LOCAL_IP);
    int bind_port_len = strlen(CMD_BIND_PORT);
    int nexthop_mac_len = strlen(CMD_NEXTHOP_MAC);
    int vlan_id_len = strlen(CMD_VLAN_ID);
    uint8_t mac[6] = {0};
    bool is_valid_remote_ip = false;
    bool is_valid_local_ip = false;
    bool is_valid_bind_port = false;
    bool is_valid_nexthop_mac = false;

    n_settings = 10; /* work around, we assume the maximum count of arguments in the add-port command will not exceed 10 */

    for (i = 0; i < n_settings; i++) {
        if (settings[i]) {
            if (!strcmp(settings[i], CMD_DASH)) {
                if (CMD_STATE_READY == state) {
                    state = CMD_STATE_START;
                }
                continue;
            }

            if (!strcmp(settings[i], CMD_SET)) {
                if (CMD_STATE_START == state) {
                    state = CMD_STATE_OP_NAME;
                }
                continue;
            }

            if (!strcmp(settings[i], CMD_INTERFACE)) {
                if (CMD_STATE_OP_NAME == state) {
                    state = CMD_STATE_DB_NAME;
                }
                continue;
            }

            if (CMD_STATE_DB_NAME == state) {
                iface_name = settings[i];
                if (false == is_valid_gre_name(iface_name)) {
                    vsctl_fatal("invalid interface name %s, only gre1-gre%u is allowed", iface_name, OFP_TUNNEL_PORT_NO_NUM);
                    return false;
                }
                state = CMD_STATE_ARGUMENTS;
                continue;
            }

            if (CMD_STATE_ARGUMENTS != state) {
                continue;
            }

            if (!netdev_type) {
                netdev_type = strstr(settings[i], CMD_NETDEV_TYPE);
                if (netdev_type && strlen(settings[i]) > netdev_type_len) {
                    netdev_type = netdev_type + netdev_type_len;
                    if (strcmp(netdev_type, "gre")) {
                        vsctl_fatal("%s: bad netdev type: %s, should be 'gre'", iface_name, netdev_type);
                        return false;
                    }
                    continue;
                }
            ...
            if (!netdev_type) {
                vsctl_fatal("%s: netdev type must be specified", iface_name);
                return false;
            }

            remote_ip = strstr(settings[i], CMD_REMOTE_IP);
            if (remote_ip && strlen(settings[i]) > remote_ip_len) {
                struct in_addr in_addr;
                remote_ip = remote_ip + remote_ip_len;
                if (false == is_valid_ipv4_address(remote_ip) || lookup_ip(remote_ip, &in_addr)) {
                    vsctl_fatal("%s: bad %s 'remote_ip'", iface_name, netdev_type);
                    return false;
                }
                is_valid_remote_ip = true;
                continue;
            }

            local_ip = strstr(settings[i], CMD_LOCAL_IP);
            if (local_ip && strlen(settings[i]) > local_ip_len) {
                struct in_addr in_addr;
                local_ip = local_ip + local_ip_len;
                if (false == is_valid_ipv4_address(local_ip) || lookup_ip(local_ip, &in_addr)) {
                    vsctl_fatal("%s: bad %s 'local_ip'", iface_name, netdev_type);
                    return false;
                }
                is_valid_local_ip = true;
                continue;
            }

            bind_port = strstr(settings[i], CMD_BIND_PORT);
            if (bind_port && strlen(settings[i]) > bind_port_len) {
                bind_port = bind_port + bind_port_len;
                if (false == is_valid_eth_name(bind_port)) {
                    vsctl_fatal("%s: bad %s 'bind_port', must be a valid physical port name, eg. eth-0-1", iface_name, netdev_type);
                    return false;
                }
                is_valid_bind_port = true;
                continue;
            }

            nexthop_mac = strstr(settings[i], CMD_NEXTHOP_MAC);
            if (nexthop_mac && strlen(settings[i]) > nexthop_mac_len) {
                nexthop_mac = nexthop_mac + nexthop_mac_len;
                if (get_mac_addr(nexthop_mac, mac)) {
                    vsctl_fatal("%s: bad %s 'nexthop_mac', should be in format '0.0.1' or 'ffff.ffff.ffff'", iface_name, netdev_type);
                    return false;
                }
                is_valid_nexthop_mac = true;
                continue;
            }
            /* set gre vlan id is optional */
            vlan_id_str = strstr(settings[i], CMD_VLAN_ID);
            if (vlan_id_str && strlen(settings[i]) > vlan_id_len) {
                vlan_id_str = vlan_id_str + vlan_id_len;
                if (strspn(vlan_id_str, "0123456789") != strlen(vlan_id_str)) {
                    vsctl_fatal("%s: bad %s 'vlan_id', should be a decimal integer", iface_name, netdev_type);
                    return false;
                }
                vlan_id = atoi(vlan_id_str);
                if (vlan_id < 1 || vlan_id > MAX_VLAN_NUM) {
                    vsctl_fatal("%s: bad %s 'vlan_id', should be in range <1-4094>", iface_name, netdev_type);
                    return false;
                }
                continue;
            }
        } else {
            if (!iface_name || !netdev_type || !is_valid_remote_ip || !is_valid_local_ip || !is_valid_bind_port || !is_valid_nexthop_mac) {
                vsctl_fatal("You must specify valid interface_name, netdev_type, remote_ip, local_ip, bind_port and nexthop_mac");
                return false;
            }
            break;
        }
...
#ifdef _OFP_CENTEC_
    if (false == is_valid_eth_name(port_name) && false == is_valid_gre_name(port_name)
            && 0 != strcmp(DEFAULT_BRIDGE_INTERFACE_NAME, port_name)
            && 0 != strcmp(DEFAULT_NETDEV_CPU_PORT, port_name)) {
        vsctl_fatal("invalid port name %s, only gre1-gre%u is allowed", port_name, OFP_TUNNEL_PORT_NO_NUM);
        return;
    }

    if (true == is_valid_gre_name(port_name) && false == check_gre_iface_settings(settings, n_settings)) {
        return;
    }
#endif
