...
struct oftable {
    ...
    struct list properties;  /*A list of table feature properties*/
#endif
};
...
#ifdef _OFP_CENTEC_
/* A group within a "struct ofgroup".
 *
 * With few exceptions, ofproto implementations may look at these fields but
 * should not modify them. */
struct ofgroup{
...
/* ## --------------------------- ## */
    ...
    /* Helper for the OpenFlow OFPT_FEATURES_REQUEST request.
     *
     * The implementation should store in '*capabilities' a bitmap of the 
     * supported OpenFlow actions. */
    void (*get_capabilities)(struct ofproto *ofproto,
                             enum ofp_version version, enum ofputil_capabilities *capabilities);

    /* Helper for fetching OpenFlow datapath id. */
    void (*get_datapath_id)(const struct ofproto *ofproto, 
                            uint64_t *datapath_id);

    /* Helper for notifying datapath the initial sync of bridge configuration
     * in ovsdb have been completed. */
    void (*initial_cfg_done)(const struct ofproto *ofproto);
#endif
...
#ifdef _OFP_CENTEC_
    /* Clear statistics for 'rule', clearing the number of packets that have
     * matched it in '*packet_count' and the number of bytes in those packets
     * in '*byte_count'.  */
    void (*rule_clear_stats)(struct rule *rule);
#endif
 ...
 * widespread use, we will delete these interfaces. */
    ...
    int (*queue_get_config)(uint16_t ofport, uint8_t queue_id, uint16_t *min_rate, uint16_t *max_rate);
#endif
};
