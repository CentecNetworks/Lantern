...
#ifdef _OFP_CENTEC_    
static enum ofperr
add_group(struct ofproto *ofproto, struct ofconn *ofconn,
         const struct ofputil_group_mod *gm, const struct ofp_header *request);
static enum ofperr
delete_group__(struct ofproto *ofproto, uint32_t group_id);

/* This live class will be initialized by provider implementation. */
struct ofproto_class ofproto_live_class;
#endif
...
#ifdef _OFP_CENTEC_    
    ofproto_class_register(&ofproto_live_class);
#endif
...
#ifdef _OFP_CENTEC_
    ofproto->ofproto_class->get_capabilities(ofproto, oh->version, &(features.capabilities));
#else
...
#ifdef _OFP_CENTEC_
    /* Not support update_port_config for GRE port */
    if (!strcmp("gre", netdev_get_type(port->netdev))) {
        return;
    }
#endif
...
#ifdef _OFP_CENTEC_
/* The use of OFPTT_ALL(0xff) is only valid for delete requests
  * for add or modify request, the switch must send error message [spec13]*/
#if 0
static enum ofperr
check_valid_table_id(const struct ofproto *ofproto, uint8_t table_id)
{
    return (table_id != 0xff && table_id < ofproto->n_tables
            ? 0
            : OFPERR_OFPBRC_BAD_TABLE_ID);
                           ...
                           const struct ofp_header *rq)
{
    ...
    if ((oqsr.queue_id != OFPQ_ALL) 
        && (oqsr.queue_id < 0 || oqsr.queue_id >= OFUTIL_MAX_QUEUE_VALUE)) {
...
#else
    error = check_valid_table_id(ofproto, fm->table_id);
#endif  /*_OFP_CENTEC_*/
    if (error) {
        return error;
    }
...
#ifdef _OFP_CENTEC_
    if (fm->flags & OFPFF12_RESET_COUNTS){
        ofproto->ofproto_class->rule_clear_stats(rule);
    }
#else
...
#ifdef _OFP_CENTEC_
void 
ofproto_table_property_add_instruction(struct ofputil_table_feature_prop *otfp, 
                        enum ofp13_instruction_type type)
{
    struct ofp11_instruction *header;
    
    header = (struct ofp11_instruction *)(otfp->elements + otfp->elements_len);
    header->len = htons(4);
    header->type = htons(type);
    otfp->elements_len = otfp->elements_len + 4;
}

void 
ofproto_table_property_add_action(struct ofputil_table_feature_prop *otfp, 
                        enum ofp11_action_type type)
{
    struct ofp_action_header *header;
    
    header = (struct ofp_action_header *)(otfp->elements + otfp->elements_len);
    header->len = htons(4);
    header->type = htons(type);
    otfp->elements_len = otfp->elements_len + 4;
}
...
void 
ofproto_table_property_add_nxaction(struct ofputil_table_feature_prop *otfp, 
                        enum nx_action_subtype type)
{
    struct nx_action_header *header;
    
    header = (struct nx_action_header *)(otfp->elements + otfp->elements_len);
    header->len = htons(10);
    header->type = htons(OFPAT11_EXPERIMENTER);
    header->vendor = htonl(NX_VENDOR_ID);
    header->subtype = htons(type);
    otfp->elements_len = otfp->elements_len + 10;
}

/* Shared by table property 'match', 'wildcards', 'apply_setfield'. */
void 
ofproto_table_property_add_oxm(struct ofputil_table_feature_prop *otfp, 
                               uint32_t oxm_header)
{
    uint32_t *header;

    /* OF Spec 1.3.1, A 3.5.5:
     ...
     * We DO NOT support experimenter OXM fields, so we can safe write 32-bit 
     * header. 
     */

    if ((oxm_header >> 16) == OFPXMC12_EXPERIMENTER) {
        NOT_REACHED();
    }
    
    header = (uint32_t *)(otfp->elements + otfp->elements_len);
    /* XXX_OF1.3 HACK, set the oxm length to 0 to make controller happy. */
    oxm_header = oxm_header & 0xffffff00;
    *header = htonl(oxm_header);    
    otfp->elements_len = otfp->elements_len + 4;
}

static struct ofputil_table_feature_prop *
ofproto_table_property_lookup(struct oftable *table, 
                          enum ofp13_table_feature_prop_type type)
{
    struct ofputil_table_feature_prop *otfp;
    LIST_FOR_EACH (otfp, list_node, &table->properties) {
        if (otfp->type == type) {
            return otfp;
        }
    }
...
struct ofputil_table_feature_prop * 
ofproto_init_table_feature_property(struct oftable *table, 
                                    enum ofp13_table_feature_prop_type type,
                                    uint32_t elements_buf_len)
{
    struct ofputil_table_feature_prop *otfp;

    otfp = ofproto_table_property_lookup(table, type);
    if (otfp == NULL) {
        otfp = xzalloc(sizeof *otfp);
        otfp->type = type;
        otfp->elements = xzalloc(elements_buf_len);
        if (otfp->elements == NULL) {
            free(otfp);
            return NULL;
        }
        otfp->elements_len = 0;
        list_push_back(&table->properties, &otfp->list_node);
    }
...
static void
ofproto_destroy_table_feature_property(struct oftable *table)
{
    struct list *list_node;
    struct ofputil_table_feature_prop *otfp;
    
    while(!list_is_empty(&table->properties)) {
        list_node = list_pop_front(&table->properties);
        otfp = (struct ofputil_table_feature_prop *)list_node;
        free(otfp->elements);
        free(otfp);
    }
                           ...
                           const struct ofp_header *request)
{
    ...
    /* XXX: We DO NOT support modify table features. */
    
    /* XXX: FIXME WHEN IMPLEMENTING MULTIPLE TABLES.
     * It seems that current dpif linux only fill first table, and 
     * handle_table_stats_request implemented by OVS it take as an array and
     * cause inconsistence, in order to find this error when implementing
     * multiple tables, we only process table 0 here.
     * */
    const struct oftable *table = &p->tables[0];
    
    p->ofproto_class->get_tables(p, &ots);

    ofpmp_init(&replies, request);
    
    otf = xmalloc(sizeof *otf);

    otf->table_id = 0;
    sprintf(otf->name, "table0");
    /* XXX: FIX ME, this behavior must be changed to drop later. */
    otf->config = OFPTC11_TABLE_MISS_CONTROLLER;
    otf->max_entries = ntohl(ots.max_entries); /* Convert back to host. */
    otf->metadata_match = table->metadata_match;
    otf->metadata_write = table->metadata_write;
    
    otf->properties = (struct list *) &table->properties;
    ofputil_append_table_feature_reply(otf, &replies);
...
append_group_stats(struct ofgroup *group, struct list *replies)
{
    ...
    calc_flow_duration__(group->created, now, &ogs.duration_sec,
                         &ogs.duration_nsec);
...
static enum ofperr
handle_group_desc_request(struct ofconn *ofconn,
                           const struct ofp_header *request)
{
    struct ofproto *ofproto = ofconn_get_ofproto(ofconn);
    struct list replies;
...
static enum ofperr
handle_group_features_request(struct ofconn *ofconn,
                           const struct ofp_header *request)
{
    struct ofproto *p = ofconn_get_ofproto(ofconn);
     struct ofpbuf *msg;
...
static int
group_map_group_buckets(const struct ofputil_group_mod *gm, struct ofgroup *ofgroup)
{
    struct list *list_node;
    struct list *bucket_list = gm->buckets;
    
    list_init(&ofgroup->buckets);
    
    ofgroup->n_buckets = 0;
    while(!list_is_empty(bucket_list)) {
        list_node = list_pop_front(bucket_list);
 ...
 * if any. */
static enum ofperr
add_group(struct ofproto *ofproto, struct ofconn *ofconn OVS_UNUSED,
          const struct ofputil_group_mod *gm, 
          const struct ofp_header *request OVS_UNUSED)
{
    int error;
    struct ofgroup *ofgroup;
 ...
 * if any. */
static enum ofperr
modify_group(struct ofproto *ofproto, struct ofconn *ofconn,
                   const struct ofputil_group_mod *gm,
                   const struct ofp_header *request)
{
    struct ofgroup *ofgroup;
    ...
    ofconn = ofconn; /* TODO not used parameter */
    request = request; /* TODO not used parameter */
...
static enum ofperr
delete_group(struct ofproto *ofproto, struct ofconn *ofconn OVS_UNUSED,
             const struct ofputil_group_mod *gm,
             const struct ofp_header *request OVS_UNUSED)
{
    ...
    if (gm->group_id == OFPG_ALL) {
        /* indirect group should be removed firstly, to make all other groups removed success */
        HMAP_FOR_EACH_SAFE (ofgroup, next_ofgroup, hmap_node, &ofproto->groups) {
            if (OFPGT11_INDIRECT == ofgroup->type) {
...
handle_group_mod(struct ofconn *ofconn, const struct ofp_header *oh)
{
    ...
    switch (gm.command) {
    case OFPGC11_ADD:
        error = add_group(ofproto, ofconn, &gm, oh);
        break;

    case OFPGC11_MODIFY:
        error = modify_group(ofproto, ofconn, &gm, oh);
        break;

    case OFPGC11_DELETE:
        error = delete_group(ofproto, ofconn, &gm, oh);
        break;
...
handle_table_mod(struct ofconn *ofconn, const struct ofp_header *oh)
{
    ...
    /*XXX: todo, openflow 1.3 do not define it.*/
                          ...
                          struct ofmeter* meter, struct list* replies)
{
    ...
    calc_flow_duration__(meter->created, time_msec(), &ms.duration_sec,
                     &ms.duration_nsec);
...
static int
netdev_get_queue_config(struct ofport *port, 
                            u_int32_t queue_id, struct ofputil_queue_config_s *queue_config)
{
    uint16_t min_rate;
    uint16_t max_rate;

    struct ofproto *ofproto = port->ofproto;
    uint16_t ofp_port = port->ofp_port;         

    if (ofproto->ofproto_class->queue_get_config) {
        if (ofproto->ofproto_class->queue_get_config(ofp_port, queue_id, &min_rate, &max_rate))
            return OFPERR_OFPQOFC_BAD_PORT;
    }

    queue_config->min_rate[queue_id] = min_rate;
    queue_config->max_rate[queue_id] = max_rate;

    return 0;
}

static void
ofputil_append_queue_config(struct ofpbuf *msgbuf,
                         const struct ofputil_queue_config_s *queue_config)
{
    int32_t queue_id;
    struct ofp_header *oh = msgbuf->data;
    struct ofp_packet_queue *queue = NULL;
    struct ofp_queue_prop_min_rate *min_rate = NULL, *max_rate = NULL;
    
    switch ((enum ofp_version)oh->version) {
    case OFP13_VERSION:
    case OFP12_VERSION:
    case OFP11_VERSION: {
        struct ofputil_queue_get_config_reply *reply = ofpbuf_put_zeros(msgbuf, sizeof *reply);
        reply->port = htonl(queue_config->port_id);

        for (queue_id = 0; queue_id < OFUTIL_MAX_QUEUE_VALUE; queue_id++) {
            queue = ofpbuf_put_zeros(msgbuf, sizeof *queue);
            queue->queue_id = htonl(queue_id);

            min_rate = ofpbuf_put_zeros(msgbuf, sizeof *min_rate);
            min_rate->rate = htons(queue_config->min_rate[queue_id]);
            min_rate->prop_header.property = htons(OFPQT_MIN_RATE);

            max_rate = ofpbuf_put_zeros(msgbuf, sizeof *max_rate);
            max_rate->rate = htons(queue_config->max_rate[queue_id]);
            max_rate->prop_header.property = htons(OFPQT_MAX_RATE);
        }
...
static enum ofperr
handle_queue_config_for_port(struct ofport *port, struct ofpbuf *msgbuf)
{
    u_int32_t qid;
    struct ofputil_queue_config_s  ofp_queue_config;

    memset(&ofp_queue_config, 0, sizeof(ofp_queue_config));

    ofp_queue_config.port_id = port->ofp_port;
    
    for (qid = 0; qid < OFUTIL_MAX_QUEUE_VALUE; qid++) {

        if (netdev_get_queue_config(port, qid, &ofp_queue_config)) {
            return OFPERR_OFPQOFC_BAD_PORT;
        }
    }

    ofputil_append_queue_config(msgbuf, &ofp_queue_config);

    return 0;
}

static struct ofpbuf *
handle_queue_encode_reply(const struct ofp_header *header, 
                struct ofconn *ofconn, struct ofputil_queue_config_request* request)
{
    ...
    case OFP12_VERSION:
    case OFP13_VERSION:
        raw = OFPRAW_OFPT11_QUEUE_GET_CONFIG_REPLY;
        break;
    default:
        NOT_REACHED();
    }

    buf = ofpraw_alloc_xid(raw, version, header->xid, 0);
...
handle_queue_get_config_request(struct ofconn *ofconn,
                           const struct ofp_header *header)
{
    struct ofproto *ofproto = ofconn_get_ofproto(ofconn);
    struct ofport *port;
    enum ofperr error = 0;
    struct ofputil_queue_config_request config_request;
    struct ofpbuf *buf;

    COVERAGE_INC(ofproto_queue_req);

    error = ofputil_decode_queue_get_config_request(header, &config_request);
    if (error) {
        return error;
    }

    buf = handle_queue_encode_reply(header, ofconn, &config_request);
    
    if (config_request.port == OFPP_ANY) {
        error = OFPERR_OFPPMFC_BAD_PORT;
        HMAP_FOR_EACH (port, hmap_node, &ofproto->ports) {
            if (!handle_queue_config_for_port(port, buf)) {
                error = 0;
            }
        }
    } else {
        port = ofproto_get_port(ofproto, config_request.port);
        error = (port
                 ? handle_queue_config_for_port(port, buf)
                 : OFPERR_OFPQOFC_BAD_PORT);
    }
...
handle_openflow__(struct ofconn *ofconn, const struct ofpbuf *msg)
{
    ...
    case OFPTYPE_GROUP_REQUEST:
        return handle_group_request(ofconn, oh);
        
    case OFPTYPE_GROUP_DESC_REQUEST:
        return handle_group_desc_request(ofconn, oh);
        
    case OFPTYPE_GROUP_FEATURES_REQUEST:
        return handle_group_features_request(ofconn, oh);
...
pick_datapath_id(const struct ofproto *ofproto)
{
    ...
    ofproto->ofproto_class->get_datapath_id(ofproto, &datapath_id);
    if (0 == datapath_id) {
...
oftable_destroy(struct oftable *table)
{
    ...
    free(table->name);
#ifdef _OFP_CENTEC_
    ofproto_destroy_table_feature_property(table);
#endif
}
...
#ifdef _OFP_CENTEC_
void
ofproto_initial_cfg_done(struct ofproto *ofproto)
{
    if (ofproto->ofproto_class->initial_cfg_done) {
        ofproto->ofproto_class->initial_cfg_done(ofproto);        
    }
}
#endif
