...
#ifdef _OFP_CENTEC_
        {
            /* XXX: this is an special processing needed by our hybrid 
             * implementation, it's not needed and hard to be contributed
             * back to ovs. */
            static bool bridge_initial_cfg_done = false;
    
            HMAP_FOR_EACH (br, node, &all_bridges) {
                ofproto_initial_cfg_done(br->ofproto);
            }

            bridge_initial_cfg_done = true;
        }
#endif
    }
...
bridge_configure_datapath_id(struct bridge *br)
{
	...
	/* avoid using openflow interface's mac address as datapath id, reset it.
	 * we should use datapath id configured in ovsdb or mac address of 'eth0' as switch's datapath id*/
#ifdef _OFP_CENTEC_
...
/* XXX: reevaluate these implementations and removed later. */
#ifdef _OFP_CENTEC_

#define CTC_CONSOLE_PRINT(fmt, args...)                      \
{                                                              \
...
struct ofproto *
ctc_bridge_get_ofproto(char *br_name)
{
    struct bridge *br;

    br = bridge_lookup(br_name);
    if(br==NULL)
    { return NULL; }
    return br->ofproto;
}
...
const struct ovsrec_bridge *
ctc_bridge_get_ovs_cfg(char *br_name)
{
    struct bridge *br;
...
static void
ofp_debug_dump_interface(struct iface *iface)
{
    CTC_CONSOLE_PRINT("name     : %s\n", iface->name ? iface->name : "");
    CTC_CONSOLE_PRINT("ofp_port : %d\n", iface->ofp_port);
    CTC_CONSOLE_PRINT("type     : %s\n", iface->type);
    CTC_CONSOLE_PRINT("cfg      : 0x%x\n", (uint32_t)iface->cfg);

}

static void
ofp_debug_dump_port(struct port *port)
{
    struct iface *iface;
    size_t iface_cnt = 0;
    ...
    /*dump interfaces*/
    CTC_CONSOLE_PRINT("  -----------iface---------------\n");
    LIST_FOR_EACH (iface, port_elem, &port->ifaces)
    {
        CTC_CONSOLE_PRINT("\nindex    : %d\n", iface_cnt++);
        ofp_debug_dump_interface(iface);
    }
    CTC_CONSOLE_PRINT("  -------end of iface -----------\n");

}

static void
ofp_debug_dump_bridge(struct bridge *br)
{
    struct port *port;
    size_t port_cnt = 0;
    ...
    CTC_CONSOLE_PRINT("name    : %s\n", br->name ? br->name : "");
    CTC_CONSOLE_PRINT("type    : %s\n", br->type ? br->type : "");
    CTC_CONSOLE_PRINT("ea      : %2x.%2x.%2x.%2x.%2x.%2x",
        br->ea[0], br->ea[1], br->ea[2],
        br->ea[3], br->ea[4], br->ea[5]);
    CTC_CONSOLE_PRINT("dft_ea  : %2x.%2x.%2x.%2x.%2x.%2x",
        br->default_ea[0], br->default_ea[1], br->default_ea[2],
        br->default_ea[3], br->default_ea[4], br->default_ea[5]);
    CTC_CONSOLE_PRINT("cfg     : 0x%x\n", (uint32_t)br->cfg);
    CTC_CONSOLE_PRINT("ofproto : 0x%x\n", (uint32_t)br->ofproto);

    /*dump bridge's all ports*/
    CTC_CONSOLE_PRINT("  -----------port---------------\n");
    HMAP_FOR_EACH (port, hmap_node, &br->ports) 
    {
        CTC_CONSOLE_PRINT("\nindex    : %d\n", port_cnt++);
        ofp_debug_dump_port(port);
    }
    CTC_CONSOLE_PRINT("  -------end of port -----------\n");

}

void
ofp_debug_dump_all_bridge(void)
{
    struct bridge *br;
    int br_cnt = 0;
