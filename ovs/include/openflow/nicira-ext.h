...
#ifdef _OFP_CENTEC_
    NXAST_PUSH_L2,              /* struct nx_action_push_l2 */
    NXAST_POP_L2,               /* struct nx_action_pop_l2 */
#endif
};
...
#ifdef _OFP_CENTEC_
/* We only need these two nicira extensions, set_mpls_label/tc/ttl will
 * be done in of 1.3 message. */
/* Action structure for NXAST_PUSH_L2. */
struct nx_action_push_l2 {
    ovs_be16 type;                  /* OFPAT_PUSH_L2. */
    ovs_be16 len;                   /* Length is 16. */
    ovs_be32 vendor;                /* NX_VENDOR_ID. */
    ovs_be16 subtype;               /* NXAST_PUSH_L2 */
    uint8_t  pad[6];
};
OFP_ASSERT(sizeof(struct nx_action_push_l2) == 16);

/* Action structure for NXAST_POP_L2. */
struct nx_action_pop_l2 {
    ovs_be16 type;                  /* OFPAT_POP_L2. */
    ovs_be16 len;                   /* Length is 16. */
    ovs_be32 vendor;                /* NX_VENDOR_ID. */
    ovs_be16 subtype;               /* NXAST_POP_L2 */
    uint8_t  pad[6];
};
OFP_ASSERT(sizeof(struct nx_action_pop_l2) == 16);
#endif
